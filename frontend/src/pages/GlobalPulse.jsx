      ...
      <Box sx={{ mb: 3 }}>
        ...
        <Grid container spacing={1.5}>
          <Grid size={{ xs: 6, sm: 4, md: 2 }}>
            <StatCard label="Dollar Index (DXY)" value={data.macro_indicators.dxy.value} change={data.macro_indicators.dxy.change_pct} />
          </Grid>
          <Grid size={{ xs: 6, sm: 4, md: 2 }}>
            <StatCard label="US 10Y Yield" value={`${data.macro_indicators.us_10y.value}%`} />
          </Grid>
          <Grid size={{ xs: 6, sm: 4, md: 2 }}>
            <StatCard label="India 10Y Yield" value={`${data.macro_indicators.india_10y.value}%`} />
          </Grid>
          <Grid size={{ xs: 6, sm: 4, md: 2 }}>
            <StatCard label="INR / USD" value={data.macro_indicators.inr_usd.value} prefix="₹" />
          </Grid>
          <Grid size={{ xs: 6, sm: 4, md: 2 }}>
              ...
              <Card sx={{ height: '100%' }}>
                <CardContent sx={{ p: 2, '&:last-child': { pb: 2 } }}>
                  ...
                  <Box sx={{ display: 'flex', alignItems: 'baseline', gap: 1, mb: 0.5 }}>
                    ...
                    <ChangeIndicator value={idx.change_pct} />
                  </Box>
        ...
        <Grid size={{ xs: 12, md: 4 }}>
          ...
          <Card>
            <CardContent>
              ...
              <Box sx={{ display: 'flex', justifyContent: 'space-between', mt: 2, pt: 2, borderTop: '1px solid rgba(255,255,255,0.06)' }}>
                <Box>
    ...
    <Card sx={{ height: '100%' }}>
      <CardContent sx={{ p: 1.5, '&:last-child': { pb: 1.5 } }}>
        ...
        <Box sx={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between' }}>
          <Typography sx={{ fontWeight: 600, fontSize: '0.95rem', fontVariantNumeric: 'tabular-nums' }}>
            {prefix}{typeof value === 'number' ? value.toLocaleString('en-IN') : value}
          </Typography>
          <ChangeIndicator value={change} fontSize="0.8rem" />
        </Box>
      </CardContent>
    </Card>
  );
...
function DepthRow({ label, value, total, color }) {
  ...
  return (
    <Box sx={{ mb: 1.5 }}>
      ...
      <Box sx={{ width: '100%', height: 6, borderRadius: 3, bgcolor: 'rgba(255,255,255,0.06)' }}>
...
function SectorChip({ sector }) {
  ...
  const bgColor = isStrong ? 'rgba(76,175,80,0.12)' : isWeak ? 'rgba(244,67,54,0.08)' : 'rgba(255,255,255,0.04)';
  const borderColor = isStrong ? 'rgba(76,175,80,0.25)' : isWeak ? 'rgba(244,67,54,0.2)' : 'rgba(255,255,255,0.08)';
